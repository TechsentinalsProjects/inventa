...
def get_best_match(title):
    ...
    # Try exact or substring match of normalized forms
    for img in images:
    ...
    # Try aggressive match based on raw words
    for img in images:
...
for i in range(len(lines)):
    ...
    if image_match and current_title:
        # If it's already an applicaion_images path, let's see if we can still try to match if we want,
        # but let's just skip it if it's already there from previous runs and just focus on old ones
        # Wait, the user might have replaced existing images. The best way is to re-evaluate ALL images!
...
print(f"Update complete, matches found: {matches_found}")
